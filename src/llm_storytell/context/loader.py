"""Context file loading and selection for pipeline runs.

Location and character selection are random per run (seeded by run_id
so the same run_id reproduces the same selection).
"""
...
class ContextSelection:
    ...
    Attributes:
        ...
        selected_characters: List of relative paths to selected character files
            (random sample of up to max_characters; max_characters=0 means all files).
...
def _seed_from_run_id(run_id: str) -> None:
    """Seed the random module from run_id for reproducible per-run selection."""
...
class ContextLoader:
    ...
    def load_context(self, run_id: str, model: str | None = None) -> ContextSelection:
        ...
        Optional (random per run, reproducible for same run_id):
        - locations/: include exactly one file (chosen at random when present).
        - world/: load all *.md in alphabetical order, fold into lore_bible
          with a clear separator; record world_files in selection.

        Args:
            run_id: Run identifier; used to seed random selection so same run_id
                yields the same location/character selection.
        ...
        _seed_from_run_id(run_id)

        # Random: one location when max_locations >= 1
        selected_location, location_content = self._select_location()

        # Random: up to _max_characters characters (at least one required)
    ...
    def _select_location(self) -> tuple[str | None, str | None]:
        """Select one location file at random when _max_locations >= 1.

        When _max_locations is 0, returns (None, None). When >= 1 and locations
        exist, selects one at random (caller must have seeded random from run_id).
