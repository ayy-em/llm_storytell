...
- **weasyprint** (>=62.0): used by the deliverable-to-book step to render HTML to PDF for no-TTS runs. Standard library cannot produce PDFs. Enables writing the final script as runs/book/{MM-DD-YY}_{app}_{model}.pdf without external binaries like pandoc.